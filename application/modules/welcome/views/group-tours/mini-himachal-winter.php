<?php
defined('BASEPATH') OR exit('No direct script access allowed');
$page_name = $this->uri->segment(1);
?>
<?php $this->load->view('frontendheader');?>
<div class="col-md-12">
					<div id="wrapper" class="wrapper bgded overlay">
<div class="slider-wrapper theme-default">
	<div id="slider" class="nivoSlider"> 
		<img src="<?php echo base_url();?>media/images/subpages/vinaya-tours-himachal.jpg?25112017" alt="Vinaya Tours - Himachal" /> 
	</div>
	<div id="htmlcaption" class="nivo-html-caption" style="display:none;"> <strong>This</strong> is an example of a <em>HTML</em> caption with <a href="#">a link</a>. </div>
</div>
  <section id="breadcrumb" class="hoc clear" style="display:none;">
<ul>
<li><a href="http://www.os-templates.com/free-website-templates">Home</a></li>
<li><a href="">About Us</a></li> 
</ul>
<h6 class="heading">About Us</h6>
</section>
</div>
</div>
<div class="container">
<div class="row">
				<div class="col-md-8 col-md-offset-2 text-center fh5co-heading margin_b0 animate-box">
					<h2>Mini Himachal Winter - 5 Night 6 Days</h2>
				</div>
			</div>
    <div class="row">
    	<div class="col-md-12">
            <div class="panel with-nav-tabs panel-warning">
                <div class="panel-heading">
                        <ul class="nav nav-tabs">
                            <li class="active tab1"><a href="#tab1warning" data-toggle="tab">Itenary</a></li>
                            <li class="tab2"><a href="#tab2warning" data-toggle="tab">Inclusion</a></li>
                            <li class="tab2"><a href="#tab3warning" data-toggle="tab">Exclusion</a></li>
                            <li class="tab4"><a href="#tab4warning" data-toggle="tab"  style="display:none;">Hotels</a></li>
                            <li class="dropdown" style="display:none;">
                                <a href="#" data-toggle="dropdown">Dropdown <span class="caret"></span></a>
                                <ul class="dropdown-menu" role="menu">
                                    <li><a href="#tab4warning" data-toggle="tab">Warning 4</a></li>
                                    <li><a href="#tab5warning" data-toggle="tab">Warning 5</a></li>
                                </ul>
                            </li>
                        </ul>
                </div>
                <div class="panel-body">
                    <div class="tab-content">
                        <div class="tab-pane fade in active" id="tab1warning">						
						<ul>
						<li><strong>Day 1</strong><br/>
						Delhi to Manali (570 Kms 14 Hrs) 
						Arrive In Delhi Go To Boarding Point (At 5:30 Pm Sharp) At 6:00 Pm where our representative will welcome you. Volvo Bus Depart From Delhi To Manali. Overnight Journey In Volvo Bus From Delhi To Manali (12 – 14 Hours). Reach In Manali Next Day Morning Around 07:30 Am.

						</li><br/>
						<li><strong>Day 2</strong>
						Arrival & Local Sightseeing
						At morning we arrive Manali, will drop you to your selected Hotel. Check In To Hotel. After Getting Fresh And Half Day Free For Leisure/rest. In Evening 03:00 PM Proceed For Manali Sightseeing Visiting To Manali Mall Road, Hadima Temple, Van Vihar, Club House, Tibetan /monastery, Hot sulfur spring Back to hotel dinner and overnight stay
						</li><br/>
						<li><strong>Day 3</strong>
						Manali Rohtang Pass /  Solang Valley 
						Morning After Getting Fresh Get Ready As Soon As Possible To Depart For Rohtang Pass. Rohtang Pass (Tourist Spot In Himalaya Having Snow All 12 Months) Is The Most Visited Tourist Spot. Rohtang Pass Situated At An Altitude Of 3979 Mtrs Above Sea Level And 51 Kms Outside Manali Is The Mighty Rohtang Pass – The Gateway To Lahaul-Spiti Valley. Whenever Rohtang Pass Is Closed Due To Bad Weather Conditions & NGT Permission, People Will Be Allowed To Visit Up To Solang Valley Which Is A Picturesque Setting; This Has Good Ski Slopes End Picnic Spots. It Has The Glacier Closest To Manali. Today You Can Do Adventure Sports Most Commonly Offered Are Snow Scooter, Skiing, Parachuting, Paragliding And Zorbing At Solang Valley And Rohtang Pass. Evening Drive Back To Hotel. Night Stay And Dinner In Hotel At Manali.
						Note:-Rohtang Pass Is Subject To Availability Due To The NGT Permission & Guest Has To Pay NGT/Manali Union Charge Directly If Any Applicable On That Time.
						</li><br/>
						<li><strong>Day 4</strong>
						Manali to Manikaran (85 km)/ Kullu (45 km) 
						Morning After Breakfast You Will Be Pickup From Hotel And Transfer To Manikaran. At 1737 Mts., On The Right Bank Of River Parvati Is Manikaran. This Place Is Famous For Hot Sulphur Springs Revered By Both Hindus & Sikhs Pilgrimages. The Water Of The Spring Is Also Said To Be Radio-Active Which Cures Many Skin Diseases. According To An Ancient Saying, Manikaran Is Also Connected With Lord Shiva And His Divine Concert Goddess Parvati. On The Way Also Visit  Vaishno Temple Which Is A Small Cave And Shrine With An Image Of Goddess Vaishno Devi. Other Temples Are Dedicated To Lord Shiva, Rama And Krishna,  And Visit Kullu Shawl Factory. On This Day You Can Also Take Enjoy Of Water Rafting At Babeli Kullu. Drive back to hotel dinner and overnight stay
						</li><br/>
						<li>
						<strong>Day 5</strong>
						Manali to Delhi :( 575 km)
						Morning after breakfast check out hotel and free time for own activities like shopping movie clubhouse Manali mall visit  etc and connect to our representative for evening drop at Volvo stand departs at 4:30 Sharp overnight journey to Delhi
						</li><br/>
						<li>
						<strong>Day 6</strong>
						Arrival in Delhi. Approx 8.00 am.
						</li>
						</ul>
						</div>
                        <div class="tab-pane fade" id="tab2warning">
						<ul>
						<li>Accommodation for 3 Nights & 4 </li>
						<li>1 Deluxe room with all amenities on double sharing basis, if extra person or extra child then above charges are applicable.</li>
						<li>Below 5 Years child will be complementary.</li>
						<li>3 Breakfast & 3 Dinner in Hotel</li>
						<li>Sight-seen - Manali - Kullu - Manikaran - Solang Valley - Manali Local Sight seen</li>
						<li>Pick from Delhi (Volvo Bus stand) & Drop to Delhi (Volvo Bus stand)</li>
						<li>All Sightseeing with Group (Tempo Traveler & Sumo Or 2 Tempo Traveler)  </li>
						<li>Delhi - Manali - Delhi AC Volvo (2 Seats)</li>
						<li>Including all taxes</li>
						<li>Hotel Check In time 12.00 noon</li>
						</ul>
						
						</div>
                        <div class="tab-pane fade" id="tab3warning"><ul>
						<li>Railway & Flight tickets are not included in this package</li>
						<li>Room Heater charges will be extra charged as per hotel rates in every hotel.</li>
						<li> plan will be veg in every hotel. (Non veg will be included on travellers request</li>
						<li>Any other meal not mentioned in Inclusion.</li>
						<li>Rohtang Pass Permit and sigh-scene will be provided as per govt. Terms and conditions.</li>
						<li>Personal expenses like soft/hard drinks, laundry etc..</li>
						<li>Fax & phone charges.</li>
						<li>Any additional transportation which is not mentioned in the itinerary.</li>
						<li>Entrance fees to Amusement parks, Boating & other theatre show charges. Monuments fee. </li>
						<li>Any Adventure activity fee is not included</li>
						<li>Not mentioned in Inclusion</li>
						<br/>
						<li><strong>Note</strong>
							<ul>
							<li>From the booking date till 30 days prior to the departure date, 30 % of the package cost is chargeable.</li>
							<li>From 30 days to 15 days prior to the departure date, 50 % of the package cost is chargeable.</li>
							<li>From 15 days to 7 days prior to departure date, 75 % of the package cost is chargeable.</li>
							<li>For cancellations made within 5 days prior to the departure date, 100% of the package cost is chargeable.</li>
							<li>No refund will be made for any unused accommodation, missed meals, transportation segments, sightseeing tours or any other service due to Bad weather, ill health, Strike, Road Blocks or any other factor beyond our Control. Such unused items are neither refundable nor exchangeable.</li>
						</ul>
						</li>
						
						</ul></div>
                        <div class="tab-pane fade" id="tab4warning">Warning 4</div>
                        <div class="tab-pane fade" id="tab5warning" style="display:none;">Warning 5</div>
                    </div>
                </div>
            </div>
        </div>
        
	</div>
</div>
<br/>
<?php $this->load->view('frontendfooter');?>